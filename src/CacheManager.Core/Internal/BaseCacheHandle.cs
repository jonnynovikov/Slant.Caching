...
public abstract class BaseCacheHandle<TCacheValue> : BaseCache<TCacheValue>
{
    ...
    /// </returns>
    protected override internal async Task<bool> AddInternal(CacheItem<TCacheValue> item)
    {
        CheckDisposed();
    ...
    /// </summary>
    /// <param name="item">The <c>CacheItem</c> to be added to the cache.</param>
    protected override internal async Task PutInternal(CacheItem<TCacheValue> item)
    {
        CheckDisposed();
        item = GetItemExpiration(item);
