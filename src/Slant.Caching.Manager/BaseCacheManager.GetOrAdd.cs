...
public partial class BaseCacheManager<TCacheValue>
{
    ...
    {
        return GetOrAdd(key, region, (k, r) => value);
    }
    ...
    public TCacheValue GetOrAdd(string key, string? region, Func<string, string, TCacheValue> valueFactory)
    {
        ...
        return GetOrAddInternal(key, region, (k, r) => new CacheItem<TCacheValue>(k, new RegionId(r), valueFactory(k, r))).Value;
    }
    ...
    /// <inheritdoc />
    public CacheItem<TCacheValue> GetOrAdd(string key, string? region,
    ...
    public bool TryGetOrAdd(string key, string? region, Func<string, string, TCacheValue> valueFactory, out TCacheValue value)
    {
        ...
        if (TryGetOrAddInternal(key, region,
                (k, r) =>
                {
                    var newValue = valueFactory(k, r);
                    return newValue == null ? null : new CacheItem<TCacheValue>(k, new RegionId(r), newValue);
                }, out var item))
        {
            value = item.Value;
            return true;
    ...
    /// <inheritdoc />
    public bool TryGetOrAdd(string key, Func<string, CacheItem<TCacheValue>> valueFactory,
    ...
    /// <inheritdoc />
    public bool TryGetOrAdd(string key, string? region, Func<string, string, CacheItem<TCacheValue>?> valueFactory,
        out CacheItem<TCacheValue>? item)
    {
        NotNullOrWhiteSpace(key, nameof(key));
        NotNullOrWhiteSpace(region, nameof(region));
    ...
    private bool TryGetOrAddInternal(string key, string? region,
        Func<string, string, CacheItem<TCacheValue>?> valueFactory, out CacheItem<TCacheValue>? item)
    {
        ...
        do
        {
            ...
            if (newItem == null) newItem = valueFactory(key, region);
    ...
    private CacheItem<TCacheValue?> GetOrAddInternal(string key, string? region,
        Func<string?, string, CacheItem<TCacheValue>> valueFactory)
    {
        CacheItem<TCacheValue?> newItem = null;
        var tries = 0;
        ...
        {
            tries++;
            CacheItem<TCacheValue?>? item = GetCacheItemInternal(key, region);
            ...
            if (AddInternal(newItem)) return newItem;
        } while (tries <= Configuration.MaxRetries);
