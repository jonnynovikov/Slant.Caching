...
/// of this library.
/// The cache manager delegates all cache operations to the list of <see cref="BaseCacheHandle{TCacheValue}"/>'s which have been
/// added. It will keep them in sync according to rules and depending on the configuration.
/// </summary>
/// <typeparam name="TCacheValue">The type of the cache value.</typeparam>
public partial class BaseCacheManager<TCacheValue> : BaseCache<TCacheValue>, ICacheManager<TCacheValue>
{
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2200:RethrowToPreserveStackDetails",
        Justification = "fine for now")]
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability",
        "CA2000:Dispose objects before losing scope", Justification = "nope")]
    private BaseCacheManager(string name, ICacheManagerConfiguration configuration)
    {
        ...
        try
        {
            ...
            if (_cacheBackplane != null) RegisterCacheBackplane(_cacheBackplane);
        }
        catch (Exception ex)
        {
    ...
    public IEnumerable<BaseCacheHandle<TCacheValue>> CacheHandles 
        => new ReadOnlyCollection<BaseCacheHandle<TCacheValue>>(new List<BaseCacheHandle<TCacheValue?>>(_cacheHandles)!);
    ...
    {
        CheckDisposed();
        if (_logTrace) Logger.LogTrace("Clear: flushing cache...");

        foreach (var handle in _cacheHandles)
        {
            if (_logTrace) Logger.LogTrace("Clear: clearing handle {0}.", handle.Configuration.Name);
        ...
        {
            if (_logTrace) Logger.LogTrace("Clear: notifies backplane.");
    ...
    public override void ClearRegion(string? region)
    {
        ...
        CheckDisposed();
        if (_logTrace) Logger.LogTrace("Clear region: {0}.", region);

        foreach (var handle in _cacheHandles)
        {
            if (_logTrace) Logger.LogTrace("Clear region: {0} in handle {1}.", region, handle.Configuration.Name);
        ...
        if (_cacheBackplane != null)
        {
            if (_logTrace) Logger.LogTrace("Clear region: {0}: notifies backplane [clear region].", region);
    ...
    public override bool Exists(string key)
    {
        foreach (var handle in _cacheHandles)
        {
            if (_logTrace) Logger.LogTrace("Checking if [{0}] exists on handle '{1}'.", key, handle.Configuration.Name);
    ...
    public override bool Exists(string key, string? region)
    {
        ...
        {
            if (_logTrace)
                Logger.LogTrace("Checking if [{0}:{1}] exists on handle '{2}'.", region, key,
    ...
    /// <inheritdoc />
    protected override internal bool AddInternal(CacheItem<TCacheValue> item)
    {
        NotNull(item, nameof(item));
        CheckDisposed();
        
        if (_logTrace) Logger.LogTrace("Add [{0}] started.", item);
        ...
        EvictFromOtherHandles(item.Key, item.Region.Value, handleIndex);
        ...
        if (result)
        {
            ...
            if (_cacheBackplane != null)
            {
                if (!item.Region.HasValue)
                    ...
                    _cacheBackplane.NotifyChange(item.Key, item.Region.Value, CacheItemChangedEventAction.Add);

                if (_logTrace) Logger.LogTrace("Notified backplane 'change' because [{0}] was added.", item);
            }

            // trigger only once and not per handle and only if the item was added!
            TriggerOnAdd(item.Key, item.Region.Value);
        }
    ...
    /// <inheritdoc />
    protected override internal void PutInternal(CacheItem<TCacheValue> item)
    {
        NotNull(item, nameof(item));

        CheckDisposed();
        if (_logTrace) Logger.LogTrace("Put [{0}] started.", item);
        ...
        foreach (var handle in _cacheHandles)
        {
            if (handle.Configuration.EnableStatistics)
            {
                ...
                var oldItem = item.Region.HasValue 
                    ? handle.GetCacheItem(item.Key, item.Region.Value) 
        ...
        {
            if (_logTrace)
                Logger.LogTrace("Put [{0}:{1}] was scuccessful. Notifying backplane [change].", item.Region,
                    item.Key);

            if (!item.Region.HasValue)
                _cacheBackplane.NotifyChange(item.Key, CacheItemChangedEventAction.Put);
            else
                _cacheBackplane.NotifyChange(item.Key, item.Region.Value, CacheItemChangedEventAction.Put);
        }

        TriggerOnPut(item.Key, item.Region.Value);
    }
    ...
    protected override void Dispose(bool disposeManaged)
    {
        if (disposeManaged)
        {
            ...
            if (_cacheBackplane != null) _cacheBackplane.Dispose();
        }
    ...
    protected override CacheItem<TCacheValue>? GetCacheItemInternal(string key, string? region)
    {
        ...
        if (_logTrace) Logger.LogTrace("Get [{0}:{1}] started.", region, key);
        ...
        for (var handleIndex = 0; handleIndex < _cacheHandles.Length; handleIndex++)
        {
            ...
            {
                if (_logTrace)
                    Logger.LogTrace("Get [{0}:{1}], found in handle[{2}] '{3}'.", region, key, handleIndex,
                        handle.Configuration.Name);
            ...
            {
                if (_logTrace)
                    Logger.LogTrace("Get [{0}:{1}], item NOT found in handle[{2}] '{3}'.", region, key, handleIndex,
                        handle.Configuration.Name);
    ...
    protected override bool RemoveInternal(string key, string? region)
    {
        ...
        if (_logTrace) Logger.LogTrace("Removing [{0}:{1}].", region, key);
        ...
        if (result)
        {
            ...
            if (_cacheBackplane != null)
            {
                if (_logTrace) Logger.LogTrace("Removed [{0}:{1}], notifying backplane [remove].", region, key);
    ...
    private void EvictFromHandle(string key, string? region, BaseCacheHandle<TCacheValue> handle)
    {
        ...
        if (result) handle.Stats.OnRemove(region);
    }
    ...
    private void AddToHandles(CacheItem<TCacheValue>? item, int foundIndex)
    {
        ...
        for (var handleIndex = 0; handleIndex < _cacheHandles.Length; handleIndex++)
            ...
            {
                if (_logTrace)
                    Logger.LogTrace("Updating handles, added [{0}] to handle '{1}'.", item,
    ...
    private void AddToHandlesBelow(CacheItem<TCacheValue>? item, int foundIndex)
    {
        ...
        if (_logTrace) Logger.LogTrace("Add [{0}] to handles below handle '{1}'.", item, foundIndex);
    ...
    private void EvictFromOtherHandles(string key, string? region, int excludeIndex)
    {
        ...
        if (_logTrace)
            Logger.LogTrace("Evict [{0}:{1}] from other handles excluding handle '{2}'.", region, key,
                excludeIndex);
    ...
    private void RegisterCacheBackplane(CacheBackplane backplane)
    {
        ...
        if (_cacheHandles.Any(p => p.Configuration.IsBackplaneSource))
        {
            ...
            {
                if (_logTrace) Logger.LogTrace("Backplane event: [Remove] of {0} {1}.", args.Key, args.Region);
            ...
            {
                if (_logTrace) Logger.LogTrace("Backplane event: [Clear].");
            ...
            {
                if (_logTrace) Logger.LogTrace("Backplane event: [Clear Region] region: {0}.", args.Region);
