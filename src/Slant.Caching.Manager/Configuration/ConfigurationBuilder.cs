...
namespace Slant.Caching.Manager.Configuration;
...
public class ConfigurationBuilder : ConfigurationBuilderCachePart
{
    ...
    /// </param>
    /// <returns>The <see cref="ICacheManagerConfiguration"/>.</returns>
    public static ICacheManagerConfiguration BuildConfiguration(
        string name,
    ...
    /// </exception>
    /// <see cref="ICacheManagerConfiguration"/>
    public static ICacheManagerConfiguration LoadConfigurationFile(string? configFileName, string? sectionName,
        string? configName)
    {
        ...
        {
            ExeConfigFilename =
                configFileName // setting exe config file name, this is the one the GetSection method expects.
        };
    ...
    static internal CacheManagerConfiguration LoadFromSection(CacheManagerSection section, string? configName)
    {
        NotNullOrWhiteSpace(configName, nameof(configName));
        ...
        foreach (var def in handleDefsSection)
        {
            ...
            handleDefs.Add(
                normId,
                new CacheHandleConfiguration(def.Id, def.HandleType)
                {
        ...
        var managerCfg =
            section.CacheManagers.FirstOrDefault(p =>
                p.Name.Equals(configName, StringComparison.OrdinalIgnoreCase));
        ...
        if (string.IsNullOrWhiteSpace(managerCfg.BackplaneType))
        {
            if (!string.IsNullOrWhiteSpace(managerCfg.BackplaneName))
                throw new InvalidOperationException(
                    "Backplane type cannot be null if backplane name is specified.");
        }
        else
        {
            if (string.IsNullOrWhiteSpace(managerCfg.BackplaneName))
            ...
            EnsureNotNull(backplaneType,
                "Backplane type not found, '{0}'. Make sure to install the corresponding nuget package.",
                managerCfg.BackplaneType);
        ...
        foreach (var handleItem in managerCfg)
        {
            ...
            var handle = new CacheHandleConfiguration(handleItem.Name, handleDef.HandleType)
            {
            ...
            if (!string.IsNullOrWhiteSpace(handleItem.ExpirationMode))
                try
                {
                    handle.ExpirationMode =
                        (ExpirationMode) Enum.Parse(typeof(ExpirationMode), handleItem.ExpirationMode);
                }
                catch (ArgumentException ex)
                {
                    throw new InvalidOperationException(
                        "Invalid value '" + handleItem.ExpirationMode + "'for expiration mode", ex);
                }
        ...
        Ensure(cfg.CacheHandleConfigurations.Count > 0,
            "There are no valid cache handles linked to the cache manager configuration [{0}]", configName);
    ...
    private static TimeSpan GetTimeSpan(string timespanCfgValue, string propName)
    {
        ...
        if (!int.TryParse(hasSuffix ? normValue.Substring(0, normValue.Length - 1) : normValue, out timeoutValue))
            throw new InvalidOperationException(
                string.Format(CultureInfo.InvariantCulture,
                    "The value of the property '{1}' cannot be parsed [{0}].", timespanCfgValue, propName));
        ...
        // hours
        if (suffix.Equals(Hours, StringComparison.OrdinalIgnoreCase)) return TimeSpan.FromHours(timeoutValue);
...
public sealed class ConfigurationBuilderCacheHandlePart
{
    ...
    internal ConfigurationBuilderCacheHandlePart(CacheHandleConfiguration cfg,
        ConfigurationBuilderCachePart parentPart)
    {
        Configuration = cfg;
        _parent = parentPart;
    }
...
public class ConfigurationBuilderCachePart
{
    ...
    public ConfigurationBuilderCachePart WithBackplane(Type backplaneType, string? configurationKey,
        params object[] args)
    {
    ...
    /// </exception>
    public ConfigurationBuilderCachePart WithBackplane(Type backplaneType, string? configurationKey,
        string? channelName, params object[] args)
    {
        NotNull(backplaneType, nameof(backplaneType));
        NotNullOrWhiteSpace(configurationKey, nameof(configurationKey));
    ...
    public ConfigurationBuilderCacheHandlePart WithDictionaryHandle(
        bool isBackplaneSource = false,
        DictionaryCacheOptions? options = null)
    {
        return WithHandle(typeof(DictionaryCacheHandle<>), Guid.NewGuid().ToString("N", CultureInfo.InvariantCulture),
            isBackplaneSource, options == null ? new object[0] : new[] {options});
    }
    ...
    {
        return WithHandle(
            typeof(DictionaryCacheHandle<>), 
            handleName, 
            isBackplaneSource,
            options == null ? Array.Empty<object>() : new object[] {options});
    }
    ...
    public ConfigurationBuilderCacheHandlePart WithHandle(Type cacheHandleBaseType, string? handleName,
        bool isBackplaneSource, params object[] configurationTypes)
    {
        NotNull(cacheHandleBaseType, nameof(cacheHandleBaseType));
        NotNullOrWhiteSpace(handleName, nameof(handleName));

        var handleCfg = new CacheHandleConfiguration(handleName, handleType: cacheHandleBaseType)
        {
    ...
    public ConfigurationBuilderCacheHandlePart WithHandle(Type cacheHandleBaseType, string? handleName)
    {
        return WithHandle(cacheHandleBaseType, handleName, false);
    }
    ...
    {
        return WithHandle(cacheHandleBaseType, Guid.NewGuid().ToString("N"), false);
    }
