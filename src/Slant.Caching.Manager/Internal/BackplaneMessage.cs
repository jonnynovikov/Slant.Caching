...
using System.Linq;
using System.Text;
using static Slant.Caching.Manager.Internal.BackplaneAction;
...
public sealed class BackplaneMessage
{
    ...
    {
        return new(owner, Changed, key, changeAction);
    }
    ...
    {
        return new(owner, Changed, key, region, changeAction);
    }
    ...
    {
        return new(owner, Removed, key, region);
    }
    ...
    public static byte[] Serialize(params BackplaneMessage[] messages)
    {
        ...
        // calc size
        var size = 0;
        for (var i = 0; i < messages.Length; i++) size += MessageWriter.GetEstimatedSize(messages[i], i != 0);

        var writer = new MessageWriter(size);

        for (var i = 0; i < messages.Length; i++) SerializeMessage(writer, messages[i], i != 0);
    ...
    public static IEnumerable<BackplaneMessage> Deserialize(byte[] message, byte[]? skipOwner = null)
    {
        ...
        if (message.Length < 5) throw new ArgumentException("Invalid message");
        ...
        while (reader.HasMore()) yield return DeserializeMessage(reader, first.OwnerIdentity);
    }
    ...
    private class MessageWriter
    {
        ...
        public static int GetEstimatedSize(BackplaneMessage msg, bool skipOwner)
        {
            ...
            var size = 2; // two enums
            if (!skipOwner) size += msg.OwnerIdentity.Length * 4;
    ...
    private class MessageReader
    {
        ...
        {
            var len = ReadInt();
            if (len <= 0) throw new IndexOutOfRangeException("Invalid length for string");
