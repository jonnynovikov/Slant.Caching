...
public abstract class BaseCache<TCacheValue> : ICache<TCacheValue>
{
    ...
    {
        // null checks are done within ctor of the item
        var item = new CacheItem<TCacheValue>(key, new RegionId(region), value);
        return Add(item);
    }
    ...
    public virtual TCacheValue? Get(string key)
    {
        ...
        if (item != null)
        {
            return item.Key.Equals(key, StringComparison.OrdinalIgnoreCase) ? item.Value : default;
        }
        return default;
    }
    ...
    public virtual TCacheValue? Get(string key, string region)
    {
        ...
        if (item.Key.Equals(key, StringComparison.OrdinalIgnoreCase) &&
            item.Region.HasValue &&
            item.Region.Value.Equals(region))
            return item.Value;
