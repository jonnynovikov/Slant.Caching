...
public abstract class BaseCacheHandle<TCacheValue> : BaseCache<TCacheValue>
{
    ...
    protected BaseCacheHandle(ICacheManagerConfiguration managerConfiguration,
    ...
    /// as Get plus Put.
    /// </remarks>
    public virtual UpdateItemResult<TCacheValue> Update(string key, Func<TCacheValue, TCacheValue> updateValue,
        int maxRetries)
    {
        ...
        lock (_updateLock)
        {
            ...
            if (original == null) return UpdateItemResult.ForItemDidNotExist<TCacheValue>();

            var newValue = updateValue(original.Value);

            if (newValue == null) return UpdateItemResult.ForFactoryReturnedNull<TCacheValue>();
    ...
    /// as Get plus Put.
    /// </remarks>
    public virtual UpdateItemResult<TCacheValue> Update(string key, string? region,
    ...
    protected void TriggerCacheSpecificRemove(string key, string? region, CacheItemRemovedReason reason, object? value)
    {
        ...
        {
            Logger.LogDebug($"'{Configuration.Name}' triggered remove '{region}:{key}' because '{reason}'.");
        }
