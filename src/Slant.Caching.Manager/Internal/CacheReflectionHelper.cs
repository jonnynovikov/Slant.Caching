...
static internal class CacheReflectionHelper
{
    static internal ILoggerFactory CreateLoggerFactory(ICacheManagerConfiguration configuration)
    {
        NotNull(configuration, nameof(configuration));

        if (configuration.LoggerFactoryType == null) return new NullLoggerFactory();
    ...
    static internal ICacheSerializer CreateSerializer(ICacheManagerConfiguration configuration,
    ...
    static internal CacheBackplane CreateBackplane(ICacheManagerConfiguration configuration,
        ILoggerFactory loggerFactory)
    {
        NotNull(configuration, nameof(configuration));
        NotNull(loggerFactory, nameof(loggerFactory));
    ...
    static internal ICollection<BaseCacheHandle<TCacheValue>> CreateCacheHandles<TCacheValue>(
        ...
        ICacheSerializer serializer)
    {
        ...
        var managerConfiguration = manager.Configuration as ICacheManagerConfiguration ??
                                   throw new ArgumentException("Manager's configuration must not be null");
        ...
        {
            logger.LogInfo("Creating handle {0} of type {1}.", handleConfiguration.Name,
                handleConfiguration.HandleType);
            var handleType = handleConfiguration.HandleType;
            var requiresSerializer = false;

            requiresSerializer = handleType.GetTypeInfo().CustomAttributes
                .Any(p => p.AttributeType == typeof(RequiresSerializerAttribute));

            if (requiresSerializer && serializer == null)
                throw new InvalidOperationException(
                    $"Cache handle {handleType.FullName} requires serialization of cached values but no serializer has been configured.");
            ...
            var types = new List<object>(new object[]
                {loggerFactory, managerConfiguration, manager, handleConfiguration});
            if (handleConfiguration.ConfigurationTypes.Length > 0)
                types.AddRange(handleConfiguration.ConfigurationTypes);

            if (serializer != null) types.Add(serializer);
        ...
        if (handles.Count == 0) throw new InvalidOperationException("No cache handles defined.");
    ...
    static internal object CreateInstance(Type instanceType, object[] knownInstances)
    {
        ...
        if (!constructors.Any())
            throw new InvalidOperationException(
                string.Format(CultureInfo.InvariantCulture,
                    "No matching public non static constructor found for type {0}.", instanceType.FullName));
        ...
        foreach (var constructor in constructors)
        {
            ...
            foreach (var param in parameters)
            {
                var paramValue = instancesCopy
                    .Where(p => p != null)
                    .FirstOrDefault(p =>
                        param.ParameterType.GetTypeInfo().IsAssignableFrom(p.GetType().GetTypeInfo()));
            ...
            if (parameters.Length == args.Count) return args.ToArray();
        }
    ...
    private static IEnumerable<Type> GetGenericBaseTypes(this Type type)
    {
        ...
        if (baseType == null || !baseType.GetTypeInfo().IsGenericType) return Enumerable.Empty<Type>();

        var genericBaseType = baseType.GetTypeInfo().IsGenericTypeDefinition
            ? baseType
    ...
    private static void CheckExtends<TValid>(Type type)
    {
        ...
        {
            if (baseType == typeof(TValid)) return;
