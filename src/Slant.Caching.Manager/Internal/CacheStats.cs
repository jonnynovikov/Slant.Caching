...
public sealed class CacheStats<TCacheValue>
{
    ...
    {
        return GetStatistic(type, _nullRegionKey);
    }
    ...
    {
        if (!_isStatsEnabled) return;

        NotNull(item, nameof(item));

        foreach (var counter in GetWorkingCounters(item.Region.Value))
        {
    ...
    public void OnGet(string? region = null)
    {
        ...
        foreach (var counter in GetWorkingCounters(region)) counter.Increment(CacheStatsCounterType.GetCalls);
    }
    ...
    public void OnHit(string? region = null)
    {
        ...
        foreach (var counter in GetWorkingCounters(region)) counter.Increment(CacheStatsCounterType.Hits);
    }
    ...
    public void OnMiss(string? region = null)
    {
        ...
        foreach (var counter in GetWorkingCounters(region)) counter.Increment(CacheStatsCounterType.Misses);
    }
    ...
    public void OnPut(CacheItem<TCacheValue> item, bool itemAdded)
    {
        ...
        foreach (var counter in GetWorkingCounters(item.Region.Value))
        {
            ...
            if (itemAdded) counter.Increment(CacheStatsCounterType.Items);
        }
    }
    ...
    private CacheStatsCounter GetCounter(string key)
    {
        ...
        if (!_counters.TryGetValue(key, out var counter))
        {
            ...
            if (_counters.TryAdd(key, counter)) return counter;
