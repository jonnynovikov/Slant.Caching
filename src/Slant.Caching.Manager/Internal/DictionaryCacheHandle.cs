...
public class DictionaryCacheHandle<TCacheValue> : BaseCacheHandle<TCacheValue>
{
    ...
    public DictionaryCacheHandle(ICacheManagerConfiguration managerConfiguration,
        CacheHandleConfiguration configuration, ILoggerFactory loggerFactory)
    ...
    public DictionaryCacheHandle(ICacheManagerConfiguration managerConfiguration,
        CacheHandleConfiguration configuration, ILoggerFactory loggerFactory, DictionaryCacheOptions options)
    ...
    protected override bool AddInternalPrepared(CacheItem<TCacheValue>? item)
    {
        ...
        var key = GetKey(item.Key, item.Region.Value);
    ...
    protected override void PutInternalPrepared(CacheItem<TCacheValue?> item)
    {
        ...
        _cache[GetKey(item.Key, item.Region.Value)] = item;
    }
    ...
    private static string? GetKey(string key, string? region)
    {
        ...
        if (string.IsNullOrWhiteSpace(region)) return key;
    ...
    {
        if (item.ExpirationMode == ExpirationMode.Absolute && item.CreatedUtc.Add(item.ExpirationTimeout) < now)
            return true;
        if (item.ExpirationMode == ExpirationMode.Sliding && item.LastAccessedUtc.Add(item.ExpirationTimeout) < now)
            return true;
    ...
    private int ScanForExpiredItems()
    {
        ...
        foreach (var item in _cache.Values)
            if (IsExpired(item, now))
            {
                ...
                // fix stats
                Stats.OnRemove(item.Region.Value);
                removed++;
            }
