...
public static class TypeCache
{
    ...
    private static List<Func<string, Type>> _resolvers = new();
                ...
                if (!_types.ContainsKey(type))
                {
                    ...
                    if (_resolvers.Count > 0)
                        foreach (var resolver in _resolvers)
                            try
                            {
                                ...
                                if (result != null) typeResult = result;
                            }
                            catch
                            {
                            }
                    ...
                    if (typeResult == null)
                    {
                        ...
                        if (typeResult == null)
                            ...
                            try
                            {
                                var withoutVersion = Regex.Replace(type, @"(,.?)Version=\d+.\d+.\d+.\d+",
                                    string.Empty);
                        ...
                        if (typeResult == null)
                            ...
                            try
                            {
                                var allReplaced = Regex.Replace(type,
                                    @",(?:[^,]*).?(mscorlib|System.Private.CoreLib).(?:(?!\]|$).)*", string.Empty);
                    ...
                    if (typeResult == null)
                        throw new InvalidOperationException(
                            $"Could not load type '{type}'. Try add TypeCache.RegisterResolveType to resolve your type if the resolving continues to fail.");
