...
public static class UpdateItemResult
{
    ...
    {
        return new(null, UpdateItemResultState.ItemDidNotExist, false, 1);
    }
    ...
    {
        return new(null, UpdateItemResultState.FactoryReturnedNull, false, 1);
    }
    ...
    /// <returns>The item result.</returns>
    public static UpdateItemResult<TCacheValue> ForSuccess<TCacheValue>(CacheItem<TCacheValue>? value,
        bool conflictOccurred = false, int triesNeeded = 1)
    {
        return new(value, UpdateItemResultState.Success, conflictOccurred, triesNeeded);
    }
    ...
    {
        return new(null, UpdateItemResultState.TooManyRetries, true, triesNeeded);
    }
}
...
{
    internal UpdateItemResult(CacheItem<TCacheValue>? value, UpdateItemResultState state, bool conflictOccurred,
        int triesNeeded)
    {
        if (triesNeeded == 0)
