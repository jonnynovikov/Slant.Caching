...
namespace Slant.Caching.Manager.Utility;
...
public class ObjectPool<T> where T : class
{
    ...
    public ObjectPool(IObjectPoolPolicy<T> policy, int? maxItems = null)
    {
        ...
        if (maxItems == null || maxItems <= 0) maxItems = Environment.ProcessorCount * 2;
    ...
    public T Lease()
    {
        ...
        {
            var item = _items[i];
            if (item != null && Interlocked.CompareExchange(ref _items[i], null, item) == item) return item;
        }
    ...
    {
        if (!_policy.Return(value)) return;
