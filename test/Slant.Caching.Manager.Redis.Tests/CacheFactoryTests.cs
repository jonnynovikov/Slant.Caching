...
using Slant.Caching.Manager.SystemRuntimeCaching;
using Xunit;
...
public class CacheFactoryTests
{
    ...
    public void CacheFactory_Build_WithRedisBackplaneNoName()
    {
        ...
        // assert
        act.Should().Throw<ArgumentException>().And.ParamName.Equals("configurationKey");
    }
    ...
    public void CacheFactory_Build_WithRedisConfigurationValidateBuilder()
    {
        ...
        configuration.Endpoints.Should().BeEquivalentTo(new[]
            {new ServerEndPoint("127.0.0.1", 2323), new ServerEndPoint("nohost", 60999)});
    }
    ...
    public void CacheFactory_FromConfig_NonGeneric_C()
    {
        var cache = CacheFactory.FromConfiguration(
            ...
            ConfigurationBuilder.BuildConfiguration(cfg =>
                cfg.WithSystemRuntimeCacheHandle())) as ICacheManager<string>;
