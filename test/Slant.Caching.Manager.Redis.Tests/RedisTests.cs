...
using Slant.Caching.Manager.Redis;
...
public class RedisTests
{
    ...
    public void Redis_Verify_NoCredentialsLoggedOrThrown()
    {
        ...
        var act = () => new BaseCacheManager<string>(cfg).Put("key", "value");
    ...
    public async Task Redis_NoRaceCondition_WithUpdate()
    {
               ...
               }))
        {
            ...
            var numThreads = 5;
            var iterations = 10;
            var numInnerIterations = 10;
            var countCasModifyCalls = 0;
            ...
            result.Counter.Should().Be(numThreads * numInnerIterations * iterations,
                "counter should be exactly the expected value");
            countCasModifyCalls.Should().BeGreaterThan((int) result.Counter,
    ...
    public void Redis_Valid_CfgFile_LoadWithRedisBackplane()
    {
        // arrange
        var fileName = TestConfigurationHelper.GetCfgFileName(@"/Configuration/configuration.valid.allFeatures.config");
        var cacheName = "redisConfigFromConfig";
    ...
    public void Redis_Valid_CfgFile_LoadWithConnectionString()
    {
        // arrange
        var fileName = TestConfigurationHelper.GetCfgFileName(@"/Configuration/configuration.valid.allFeatures.config");
        var cacheName = "redisConfigFromConnectionString";
    ...
    public void Redis_LoadWithRedisBackplane_FromAppConfig()
    {
        ...
        // arrange
        var cacheName = "redisWithBackplaneAppConfig";
    ...
    public void Redis_LoadWithRedisBackplane_FromAppConfigConnectionStrings()
    {
        ...
        // arrange
        var cacheName = "redisWithBackplaneAppConfigConnectionStrings";
    ...
    public void Redis_LoadWithRedisBackplane_FromAppConfigConnectionStrings_WithDefaultDb()
    {
        // RedisConfigurations should load this from AppSettings from app.config
        // arrange
        var cacheName = "redisWithBackplaneAppConfigConnectionStringsWithDefaultDb";
    ...
    public void Redis_ValueConverter_Poco_Update()
    {
        ...
        using (cache)
        {
            ...
            var act = () => cache.TryUpdate(key, region, (o) => newValue, out resultValue);
        ...
        where TCache : ICacheManager<object>
    {
        for (var i = 0; i < iterations; i++)
        {
            ...
            foreach (var cache in caches) await stepB(cache);
        }
        ...
        where TEventArgs : EventArgs
    {
        ...
        return TestBackplaneEventRunner(cacheA, cacheB, cacheEvent, (a) => arrange(cacheA, cacheB), assertLocal,
            assertRemote, expectedRemoteTriggers);
    }
        ...
        where TEventArgs : EventArgs
    {
        ...
        {
            if (err is XunitException xunitError) return xunitError.Message;
