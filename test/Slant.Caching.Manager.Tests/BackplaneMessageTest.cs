...
public class BackplaneMessageTest
{
    ...
    public void BackplaneMessage_Multiple_IgnoreOwner()
    {
        ...
        var serialized = BackplaneMessage.Serialize(messages.ToArray());
        var deserialized = BackplaneMessage.Deserialize(serialized, owner).ToArray();
    ...
    private static ISet<BackplaneMessage> CreateMany(byte[] owner)
    {
        ...
        for (var m = 0; m < 100; m++)
        {
            ...
            for (var i = 0; i < 10; i++) messages.Add(BackplaneMessage.ForClearRegion(owner, region + i));
            for (var i = 0; i < 10; i++) messages.Add(BackplaneMessage.ForRemoved(owner, key + i, region));
        }
