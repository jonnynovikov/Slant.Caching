...
public class CacheManagerEventsTest
{
    ...
    public void Events_CacheActionEventArgsCtor()
    {
        ...
        // assert
        act.Should().Throw<ArgumentNullException>().And.ParamName.Equals("key");
    }
    ...
    public void Events_CacheActionEventArgsCtor_Valid()
    {
        ...
        // act
        var act = () => new CacheActionEventArgs(key, region);

        // assert
        act().Should().BeEquivalentTo(new
            {Region = (string) null, Key = key, Origin = CacheActionEventArgOrigin.Local});
    }
    ...
    public void Events_CacheItemRemovedEventArgsCtor_Valid()
    {
        ...
        // act
        var act = () => new CacheItemRemovedEventArgs(key, region, CacheItemRemovedReason.Expired, null, 2);

        // assert
        act().Should().BeEquivalentTo(new
        {
            Region = (string) null, Key = key, Reason = CacheItemRemovedReason.Expired, Level = 2, Value = (object) null
        });
    }
    ...
    public void Events_CacheItemRemovedEventArgsCtor_ValidB()
    {
        ...
        // act
        var act = () => new CacheItemRemovedEventArgs(key, region, CacheItemRemovedReason.Expired, "value", 2);

        // assert
        act().Should().BeEquivalentTo(new
            {Region = region, Key = key, Reason = CacheItemRemovedReason.Expired, Level = 2, Value = "value"});
    }
    ...
    public void Events_CacheItemRemovedEventArgsCtor_ValidC()
    {
        ...
        // act
        var act = () => new CacheItemRemovedEventArgs(key, region, CacheItemRemovedReason.Evicted, "value", 0);

        // assert
        act().Should().BeEquivalentTo(new
            {Region = region, Key = key, Reason = CacheItemRemovedReason.Evicted, Level = 0, Value = "value"});
    }
    ...
    public void Events_CacheClearRegionEventArgsCtor_Valid()
    {
        ...
        // act
        var act = () => new CacheClearRegionEventArgs(region);
    ...
    public class LongRunningEventTestBase
    {
            ...
            bool expectValue = true)
        {
            ...
            {
                if (args.Key.Equals(useKey) && (region.HasValue || args.Region == region.Value))
                {
                    triggered = true;
                    resultArgs = args;
                }
            ...
            if (region.HasValue)
            {
                cache.Add(useKey, "value", region.Value);
                cache.Get(useKey, region.Value).Should().NotBeNull();
            }
            else
            {
            ...
            while (count < 30 && !triggered)
            {
                if (runGetWhileWaiting)
                {
                    ...
                    {
                        cache.CacheHandles.ToList().ForEach(p => p.Get(useKey, region.Value));
                    }
                }
            ...
            if (!triggered) throw new Exception("Waited pretty long, no events triggered...");
            ...
            if (endGetShouldBeNull)
            {
                if (region.IsEmpty)
                    ...
                    cache.Get(useKey, region.Value).Should().BeNull();
            }
    ...
    public class DictionarySpecific : LongRunningEventTestBase
    {
        ...
        public async Task Events_Dic_ExpireTriggers()
        {
            ...
            var result = await RunTest(cfg, useKey, region, true, true);
        ...
        public async Task Events_Dic_ExpireEvictsAbove()
        {
            ...
            result.Key.Should().Be(useKey);
            result.Region.Should().Be(RegionId.Empty.Value);
        }
    }
    ...
    public class MsMemorySpecific : LongRunningEventTestBase
    {
        ...
        public async Task Events_MsMemory_ExpireEvictsAbove()
        {
            ...
            var result = await RunTest(cfg, useKey, RegionId.Empty, true, true);
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            data.Calls.Should().Be(8,
                $"we expect 8 hits for {key1} and {key2} \n-> keys: " + string.Join(", ", data.Keys));
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            data.Calls.Should().Be(12,
                $"we expect 12 hits for {key1} and {key2} \n-> keys: " + string.Join(", ", data.Keys));
            data.Keys.Should().BeEquivalentTo(
                Enumerable.Repeat(key1, 3)
                    .Concat(Enumerable.Repeat(key2, 3))
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            data.Calls.Should().Be(6,
                $"we expect 6 hits for {key1} and {key2} \n-> keys: " + string.Join(", ", data.Keys));
    ...
    private class EventCallbackData
    {
        ...
        internal void AddCall(CacheActionEventArgs args, params string[] validKeys)
        {
            ...
            if (validKeys.Contains(args.Key))
            {
                Calls++;
                Keys.Add(args.Key);
                if (!string.IsNullOrWhiteSpace(args.Region)) Regions.Add(args.Region);
            }
        }
    ...
    {
        public CustomRemoveEventTestHandle(ICacheManagerConfiguration managerConfiguration,
        ...
        protected override ILogger Logger => Loggers.None;
