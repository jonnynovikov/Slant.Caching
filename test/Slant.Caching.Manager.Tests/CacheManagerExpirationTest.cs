...
public class CacheManagerExpirationTest
{
            ...
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                await TestSlidingExpiration(
                    timeout,
                    (key) => cache.Add(new CacheItem<object>(key, "value", ExpirationMode.Sliding,
            ...
            using (cache)
            {
                ...
                try
                {
                    await TestSlidingExpiration(
                        ...
                        (key) =>
                        {
                            if (cache.TryUpdate(key, o => o, out var value)) return value;
    ...
    public class MsMemory
    {
        ...
        public async Task MsMemory_Absolute_DoesExpire()
        {
            ...
            using (var cache = CacheFactory.Build(_ => _.WithMicrosoftMemoryCacheHandle()))
            {
                await TestAbsoluteExpiration(
                    timeout,
                    (key) => cache.Add(new CacheItem<object>(key, "something", ExpirationMode.Absolute,
                        TimeSpan.FromMilliseconds(timeout))),
                    (key) => cache.Get(key));
            }
        }
        ...
        public async Task MsMemory_Sliding_DoesExpire()
        {
            ...
            using (var cache = CacheFactory.Build(_ => _.WithMicrosoftMemoryCacheHandle()))
            {
                await TestSlidingExpiration(
                    timeout,
                    (key) => cache.Add(new CacheItem<object>(key, "something", ExpirationMode.Sliding,
                        TimeSpan.FromMilliseconds(timeout))),
                    (key) => cache.Get(key));
            }
        }
    ...
    public class Redis
    {
        ...
        public async Task Redis_Absolute_DoesExpire_MultiClients()
        {
            ...
            using (cacheB)
            {
                await TestAbsoluteExpiration(
                    timeout,
                    (key) => cacheA.Add(new CacheItem<object>(key, "something", ExpirationMode.Absolute,
                        TimeSpan.FromMilliseconds(timeout))),
                    (key) =>
                    {
        ...
        public async Task Redis_Sliding_DoesExpire_MultiClients()
        {
            ...
            using (cacheB)
            {
                await TestSlidingExpiration(
                    timeout,
                    (key) => cacheA.Add(new CacheItem<object>(key, "something", ExpirationMode.Sliding,
                        TimeSpan.FromMilliseconds(timeout))),
                    (key) =>
                    {
                        var a = cacheA.GetCacheItem(key);
            ...
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                await TestRemoveExpiration(
                    timeout,
                    (key) => cache.Add(new CacheItem<object>(key, "value", ExpirationMode.Absolute,
                        TimeSpan.FromMilliseconds(timeout))),
                    (key) =>
                    {
                        var item = cache.GetCacheItem(key);
    ...
    public class ValidateExpire
    {
            ...
            where T : ICacheManager<object>
        {
            ...
            {
                var key = Guid.NewGuid().ToString();
                cache.Add(new CacheItem<object>(key, "value", ExpirationMode.None, default))
                    .Should().BeTrue();
                ...
                item.ExpirationTimeout.Should().BeCloseTo(TimeSpan.FromMinutes(10), 200);
            ...
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                item.ExpirationTimeout.Should().Be(default);
            ...
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                var region = Guid.NewGuid().ToString();
                cache.Add(
                        new CacheItem<object>(key, region, "value", ExpirationMode.Absolute, TimeSpan.FromMinutes(30)))
                    .Should().BeTrue();
    ...
    public void CacheItem_WithExpiration()
    {
        ...
        absolute.ExpirationTimeout.Should().BeCloseTo(TimeSpan.FromMinutes(10), 200);
        sliding.ExpirationMode.Should().Be(ExpirationMode.Sliding);
        sliding.ExpirationTimeout.Should().BeCloseTo(TimeSpan.FromMinutes(10), 200);
        none.ExpirationMode.Should().Be(ExpirationMode.None);
        none.ExpirationTimeout.Should().BeCloseTo(default);
    }
    ...
    public void CacheItem_WithNoExpiration()
    {
        ...
        absolute.ExpirationTimeout.Should().BeCloseTo(default);
    }
    ...
    public void BaseCacheHandle_ExpirationInherits_Issue_1()
    {
               ...
               }))
        {
            ...
            handles[1].GetCacheItem(key).ExpirationTimeout.Should().Be(default);
        }
    }
        ...
        foreach (var handle in cache.CacheHandles)
            if (cache.GetCacheItem(key, region) == null)
                throw new InvalidOperationException(
                    $"'{key}:{region}' doesn't exist in handle {handle.Configuration.Name}.");
    }

    private static async Task TestSlidingExpiration(int timeoutMillis, Func<string, bool> addFunc,
        Func<string, object> getFunc)
    {
        var tries = 0;
        while (true)
        {
            tries++;
            if (tries > 20) throw new Exception("Timing issues on testing sliding expiration... stopping.");
    ...
    private static async Task TestAbsoluteExpiration(int timeoutMillis, Func<string, bool> addFunc,
        Func<string, object> getFunc)
    {
        var tries = 0;
        while (true)
        {
            tries++;
            if (tries > 20) throw new Exception("Timing issues on testing absolute expiration... stopping.");
    ...
    private static async Task TestRemoveExpiration(int timeoutMillis, Func<string, bool> addFunc,
        Func<string, bool> removeFunc, Func<string, object> getFunc)
    {
        var tries = 0;
        while (true)
        {
