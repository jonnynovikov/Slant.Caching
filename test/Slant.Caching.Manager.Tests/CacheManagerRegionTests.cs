...
public class CacheManagerRegionTests
{
        ...
        where T : ICacheManager<object>
    {
        ...
        {
            // act
            AddRegionData(cache, 23, 3, true, out var keys, out var regions);
            ...
            foreach (var item in keys)
            {
                ...
                foreach (var otherRegion in otherRegions)
                    cache.Get(key, otherRegion).Should()
                        .Be(null, "the cache should not find the item in other regions");
            }
        }
    }
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            try
            {
                ...
                cache.Add("SomeNewItem",
                    "Should be added to the region and the region should be re added to the cache.", clearedRegion);
    ...
    private static void AddRegionData(ICache<object> cache, int numItems, int numRegions, bool sameKey,
        out List<Tuple<string, string, string>> keys, out List<string> regions)
    {
        ...
        for (var r = 0; r < numRegions; r++)
        {
            ...
            for (var i = 0; i < numItems; i++)
            {
                ...
                if (!cache.Add(key, value, region))
                    throw new InvalidOperationException("Adding key " + key + ":" + value +
                                                        " didn't work. For cache\n" + cache.ToString());
