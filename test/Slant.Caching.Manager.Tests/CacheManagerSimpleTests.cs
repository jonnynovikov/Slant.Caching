...
{
    private readonly RegionId _region = new("region");
    ...
    {
        var manager =
            new BaseCacheManager<DateTime?>(ConfigurationBuilder.BuildConfiguration(s => s.WithDictionaryHandle()));

        var value = new Nullable<DateTime>();
        Assert.Null(value);
    ...
    public void CacheManager_AddCacheItem_WithExpMode_ButWithoutTimeout()
    {
        ...
        // act
        Action act = () => cache.Add(new CacheItem<object>(key, "something", ExpirationMode.Absolute, default));
    ...
    public void CacheManager_CtorA_ConfigNoName()
    {
        ...
        var manager =
            new BaseCacheManager<object>(ConfigurationBuilder.BuildConfiguration(s => s.WithDictionaryHandle()));
    ...
    public void CacheManager_Put_InvalidKey()
    {
        ...
        {
            // arrange act
            var act = () => cache.Put(null, null);
            var actR = () => cache.Put(null, null, null);
    ...
    public void CacheManager_Put_InvalidValue()
    {
        ...
        {
            // arrange act
            var act = () => cache.Put("key", null);
            var actR = () => cache.Put("key", null, null);
    ...
    public void CacheManager_Put_InvalidRegion()
    {
        ...
        {
            // act
            var act = () => cache.Put("key", "value", null);
    ...
    public void CacheManager_Put_CacheItem_Positive<T>(T value)
    {
        ...
        var itemRegion = new CacheItem<object>(key, _region, value);

        // act
        var act = () => cache.Put(item);
        var actRegion = () => cache.Put(itemRegion);
    ...
    public void CacheManager_Put_KeyValue_Positive<T>(T value)
    {
               ...
               }))
        {
            ...
            // act
            var act = () => cache.Put(key, value);
            var actRegion = () => cache.Put(key, value, "region");
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            object value;
            var act2 = () => cache.TryUpdate(key, item => item, out value);
            var act2R = () => cache.TryUpdate(key, "region", item => item, out value);
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            object val = null;
            var actT = () => cache.TryUpdate(key, (v) => null, out val);
            var actTR = () => cache.TryUpdate(key, region, (v) => null, out val);
            var actTU = () => cache.TryUpdate(key, (v) => null, 33, out val);
            var actTRU = () => cache.TryUpdate(key, region, (v) => null, 33, out val);
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            // act
            var act = () => cache.Update(key, item => item + " more");
            var actR = () => cache.Update(key, region, item => item + " more");

            object value = string.Empty;
            object value2 = string.Empty;
            var actT = () => cache.TryUpdate(key, item => item + " awesome", out value);
            var actTR = () => cache.TryUpdate(key, region, item => item + " awesome", out value2);
            var act2 = () => cache.Get<string>(key);
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            // act
            var act = () => cache.AddOrUpdate(key, value, item => "not this value");
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            // act
            var act = () => cache.AddOrUpdate(key, "does exist", item =>
            {
        ...
        where T : ICacheManager<object>
    {
        ...
        using (cache)
        {
            ...
            cache.GetOrAdd(key, region.Value, val);
            cache.GetOrAdd(keyF, (k) => val);
            cache.GetOrAdd(keyF, region.Value, (k, r) => val);
            cache.GetOrAdd(keyG, (k) => new CacheItem<object>(keyG, val));
            cache.GetOrAdd(keyG, region.Value,
                (k, r) => new CacheItem<object>(keyG, region, val, ExpirationMode.Absolute, TimeSpan.FromMinutes(42)));

            // assert
            cache[key].Should().Be(val);
            cache[key, region.Value].Should().Be(val);
            cache[keyF].Should().Be(val);
            cache[keyF, region.Value].Should().Be(val);
            cache[keyG].Should().Be(val);
            cache[keyG, region.Value].Should().Be(val);
            var item = cache.GetCacheItem(keyG, region.Value);
        ...
        where T : ICacheManager<object>
    {
        ...
        {
            // act
            var actA = () => cache.TryGetOrAdd(key, k => val, out valueA);
            var actB = () => cache.TryGetOrAdd(key, region.Value, (k, r) => val, out valueB);
            var valC = new CacheItem<object>(key2, val);
            var actC = () => cache.TryGetOrAdd(key2, k => valC, out valueC);
            var valD = new CacheItem<object>(key2, region, val, ExpirationMode.Absolute, TimeSpan.FromMinutes(42));
            var actD = () => cache.TryGetOrAdd(key2, region.Value, (k, r) => valD, out valueD);
            ...
            cache[key2, region.Value].Should().Be(val);
            var item = cache.GetCacheItem(key2, region.Value);
            item.ExpirationMode.Should().Be(ExpirationMode.Absolute);
            item.ExpirationTimeout.Should().Be(TimeSpan.FromMinutes(42));
        }
    }
        ...
        where T : ICacheManager<object>
    {
        ...
        using (cache)
        {
            ...
            var act = () => cache.TryGetOrAdd(key, (k) => null, out val);
            var actB = () => cache.TryGetOrAdd(key, (k) => null, out val2);
        ...
        where T : ICacheManager<object>
    {
        ...
        using (cache)
        {
            ...
            // act
            var actA = () => cache.TryGetOrAdd(key, add, out resultA);
            var actB = () => cache.TryGetOrAdd(key, region, addRegion, out resultB);
            var actC = () => cache.TryGetOrAdd(key, region, (k, r) => cacheItem, out resultC);
        ...
        where T : ICacheManager<object>
    {
        ...
        using (cache)
        {
            ...
            for (var i = 0; i < runs; i++) tasks.Add(Task.Run(action));
            ...
            var action = () =>
            {
                ...
                while (!cache.TryGetOrAdd(
                           key, (k) =>
                           {
                               ...
                               // force collision so that multiple threads try to add... yea thats long, but parallel should be fine
                               Task.Delay(1).Wait();
                               return new CacheItem<object>(k, counter, ExpirationMode.Absolute,
                                   TimeSpan.FromMinutes(tries));
                           },
                           out result))
                {
                }
    ...
    public void CacheManager_GetAdd_Positive()
    {
               ...
               }))
        {
            ...
            // act
            var actAdd = () => cache.Add(key, value);
            var actGet = () => cache.Get(key);
    ...
    public void CacheManager_GetCacheItem_Positive()
    {
               ...
               }))
        {
            ...
            // act
            var actAdd = () => cache.Add(key, value);
            var actGet = () => cache.GetCacheItem(key);
    ...
    public void CacheManager_Remove_KeyNotAvailable()
    {
               ...
               }))
        {
            ...
            // act
            var act = () => cache.Remove(key);
            var actR = () => cache.Remove(key, "region");
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            // act
            var actA = () => cache.Add(key, "some value", region);
            var act = () => cache.Get<string>(key, region);
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            var someObject = cache.Get<object>("nonexistent");
        ...
        where T : ICacheManager<object>
    {
        using (cache)
        {
            ...
            for (var i = 0; i < keys.Count; i++) cache.Remove(keys[i]);
    ...
    private static void PopulateCache<T>(ICacheManager<T> cache, IList<string> keys, IList<T> values, int mode)
    {
        ...
        lock (runLock)
        {
            ...
            for (var i = 0; i < values.Count; i++)
            {
                ...
                if (val != null) throw new InvalidOperationException("cache already contains this element");

                if (mode == 0)
                    cache.Put(keys[i], values[i]);
                else if (mode == 1)
                    cache.Add(keys[i], values[i]).Should().BeTrue();
                else if (mode == 2) cache[keys[i]] = values[i];
            }
        }
    }
    ...
    public class ComplexType
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            return Name.Equals(target.Name) && SomeBool.Equals(target.SomeBool) && SomeId.Equals(target.SomeId);
        }
