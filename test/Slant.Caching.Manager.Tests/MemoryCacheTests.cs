...
public class MemoryCacheTests
{
    ...
    public void MsMemory_Extensions_SimpleWithCfg()
    {
        ...
        cache.CacheHandles.Count().Should().Be(1);
        cache.CacheHandles.OfType<MemoryCacheHandle<string>>().First().MemoryCacheOptions.Should()
            .BeEquivalentTo(expectedCacheOptions);
    }
    ...
    public void SysRuntime_Extensions_NamedWithCodeCfg()
    {
        ...
        using (var act = CacheFactory.Build(_ =>
                   _.WithSystemRuntimeCacheHandle("NamedTestWithCfg", expectedCacheOptions)))
        {
            // arrange
            var settings = ((CacheManager.SystemRuntimeCaching.MemoryCacheHandle<object>) act.CacheHandles.ElementAt(0))
                .CacheSettings;

            // act assert
            settings["CacheMemoryLimitMegabytes"].Should()
                .Be(expectedCacheOptions.CacheMemoryLimitMegabytes.ToString(CultureInfo.InvariantCulture));
            settings["PhysicalMemoryLimitPercentage"].Should()
                .Be(expectedCacheOptions.PhysicalMemoryLimitPercentage.ToString(CultureInfo.InvariantCulture));
        ...
        for (var i = 0; i < 100; i++)
        {
            var act = () =>
            {
                try
                {
                    ...
                    if (val == null) _cache.Put("some_key", "value");
                }
