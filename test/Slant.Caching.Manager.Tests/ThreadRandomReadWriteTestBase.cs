    ...
    public async Task Thread_Update(ICacheManager<object> cache)
    {
        using (cache)
        {
            ...
            var handleInfo = string.Join("\nh: ",
                cache.CacheHandles.Select(p => p.Configuration.Name + ":" + p.GetType().Name));
                    ...
                    for (var i = 0; i < numInnerIterations; i++)
                        cache.Update(
                            ...
                            (value) =>
                            {
                                ...
                                if (val == null) throw new InvalidOperationException("WTF invalid object result");
